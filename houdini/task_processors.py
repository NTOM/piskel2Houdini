...
class RoomGenerationProcessor(BaseTaskProcessor):
    ...
    def _resolve_hython_path(self, payload: Dict[str, Any]) -> str:
        """解析 hython 可执行文件路径。"""
    ...
    def _normalize_parms(self, parms: dict) -> dict:
        """确保参数字典中的所有键都转换为小写。"""
        if not parms:
            return {}
        return {k.lower(): v for k, v in parms.items()}
    
    def _extract_uuid(self, payload: Dict[str, Any], parms_lower: Dict[str, Any]) -> str:
        """从请求中提取 UUID。"""
    ...
    def _log_path_for_hip(self, hip_path: str, uuid_val: str) -> Optional[str]:
        """计算日志文件路径。"""
        try:
    ...
    def _write_json_safely(self, path: Optional[str], data: Dict[str, Any]) -> None:
        """安全写入JSON文件。"""
        if not path:
            return
        try:
    ...
    def execute(self, payload: dict) -> dict:
        ...
        try:
            ...
            # 解析 hython 路径
            hython_path = self._resolve_hython_path(payload)
            ...
            job = {
                ...
                "cook_node": cook_node, 
                "parm_node": parm_node, 
                "parms": self._normalize_parms(parms)
            }
            ...
            # 执行后置处理（JSON→PNG）
            uuid_val = self._extract_uuid(payload, self._normalize_parms(parms))
            post_info = None
            
            if proc.returncode == 0 and worker_json and worker_json.get("ok") and uuid_val:
                ...
                if os.path.isfile(reader):
                    ...
                    try:
                        ...
                        elapsed_ms_post = round((time.time() - start_post) * 1000)
                        stdout_post = (proc_post.stdout or "").strip()
                        stderr_post = (proc_post.stderr or "").strip()
                        post_json = None
                        if stdout_post:
                            try:
                        ...
                        post_info = {
                            ...
                            "stderr": "",
                            "json": post_json,
                            "elapsed_ms_post": elapsed_ms_post,
                            "ok": bool(post_json and post_json.get("ok") and proc_post.returncode == 0)
                        }
            ...
            self._write_json_safely(self._log_path_for_hip(hip, uuid_val), log_obj)
